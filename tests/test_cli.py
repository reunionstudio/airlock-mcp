...
class CliTests(unittest.TestCase):
    ...
    def test_summary_and_export_csv(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("file_rules:", summary)
            self.assertIn("attachments:", summary)
            self.assertIn("guest_access:", summary)
            self.assertIn("summary: shared public folder: append_access, read_access", summary)
            self.assertIn("column_rules:", summary)
            self.assertIn("samples:", summary)
            self.assertIn("first_record_key: post_id=POST-001", summary)
    ...
    def test_init_repo_bootstraps_codex_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Keep this specs project in git", output)
